...
class DataGenerator:
    def __init__(self, datas: List, tokenizer: object, rel2id: Dict, all_rels: List, max_len: int,
        ...
        for data in datas:
            ...
            # hc
            obj_set = set()  # (obj head, obj tail)
            obj_rel_set = set()  # (obj head, obj tail, rel)
            trans_map_subj = defaultdict(list)   # {(obj_head, rel): [tail_heads]}
            ...
            for triple in data['triple_list']:
                ...
                # hc
                obj_rel_set.add((obj_head_idx, obj_tail_idx, rel_idx))

                subj_set.add((subj_head_idx, subj_tail_idx))
                # hc
                obj_set.add((obj_head_idx, obj_tail_idx))
                ...
                # hc
                trans_map_subj[(obj_head_idx, obj_tail_idx, rel_idx)].append(subj_head_idx)
            ...
            for subj_head_idx, subj_tail_idx, obj_head_idx, obj_tail_idx, rel_idx in triples_list:
                ...
                # hc
                sample_subj_heads = np.zeros(self.max_len)
                for idx in trans_map_subj[(obj_head_idx, obj_tail_idx, rel_idx)]:
                    sample_subj_heads[idx] = 1.0
                ...
                pos_datas.append({
                    ...
                    'sample_obj_head': obj_head_idx,
                    'sample_obj_tail': obj_tail_idx,
                    'sample_subj_heads': sample_subj_heads,
                })


                neg_subj_head_idx = obj_head_idx
                neg_subj_tail_idx = obj_tail_idx
                neg_pair = (neg_subj_head_idx, neg_subj_tail_idx, rel_idx)
                ...
                if neg_pair not in subj_rel_set and neg_pair not in neg_history:
                    current_neg_datas.append({
                        ...
                        'sample_subj_tail': neg_subj_tail_idx,
                        'sample_rel': rel_idx,
                        'sample_obj_heads': np.zeros(self.max_len),
                        'sample_obj_head': neg_subj_head_idx,
                        'sample_obj_tail': neg_subj_tail_idx,
                        'sample_subj_heads': np.zeros(self.max_len),
                    })
                ...
                for neg_rel_idx in rel_set - {rel_idx}:
                    ...
                    if neg_pair not in subj_rel_set and neg_pair not in neg_history:
                        current_neg_datas.append({
                            ...
                            'sample_obj_heads': np.zeros(self.max_len),
                            'sample_obj_head': subj_head_idx,
                            'sample_obj_tail': subj_tail_idx,
                            'sample_subj_heads': np.zeros(self.max_len),
                        })
                        neg_history.add(neg_pair)


                for (neg_subj_head_idx, neg_subj_tail_idx) in subj_set - {(subj_head_idx, subj_tail_idx)}:
                    neg_pair = (neg_subj_head_idx, neg_subj_tail_idx, rel_idx)
                    if neg_pair not in subj_rel_set and neg_pair not in neg_history:
                        current_neg_datas.append({
                            'token_ids': text_tokened.ids,
    ...
    def __iter__(self, random: bool = False):
        ...
        # hc
        batch_sample_obj_head, batch_sample_obj_tail = [], []
        batch_sample_subj_heads = []
        ...
        for idx in idxs:
            ...
            # hc
            batch_sample_obj_head.append(obj['sample_obj_head'])
            batch_sample_obj_tail.append(obj['sample_obj_tail'])
            batch_sample_subj_heads.append(obj['sample_subj_heads'])
            if len(batch_tokens) == self.batch_size or idx == idxs[-1]:
                ...
                # hc
                batch_sample_obj_head = np.array(batch_sample_obj_head)
                batch_sample_obj_tail = np.array(batch_sample_obj_tail)
                batch_sample_subj_heads = np.array(batch_sample_subj_heads)
                ...
                # hc
                yield [batch_tokens, batch_segments, batch_entity_heads, batch_entity_tails, batch_rels, batch_sample_subj_head, batch_sample_subj_tail, batch_sample_rel, batch_sample_obj_heads, batch_sample_obj_head, batch_sample_obj_tail, batch_sample_subj_heads], None
                batch_tokens, batch_segments = [], []
                batch_entity_heads, batch_entity_tails = [], []
                batch_rels = []
                batch_sample_subj_head, batch_sample_subj_tail = [], []
                ...
                batch_sample_obj_heads = []
                # hc
                batch_sample_obj_head = []
                batch_sample_obj_tail = []
                batch_sample_subj_heads = []
