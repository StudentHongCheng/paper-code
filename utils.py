...
class Infer:
    # def __init__(self, entity_model: Models, rel_model: Models, translate_model: Models,tokenizer: object, id2rel: Dict):
    def __init__(self, entity_model: Models, rel_model: Models, translate_model: Models, translate_model_sub: Models, tokenizer: object, id2rel: Dict):
        self.entity_model = entity_model
        self.rel_model = rel_model
        self.translate_model = translate_model
        self.translate_model_sub = translate_model_sub
    ...
    def __call__(self, text: str, threshold: float = 0.5) -> Set:
        ...
        for head, head_type in zip(*entity_heads):
            for tail, tail_type in zip(*entity_tails):
                if head <= tail and head_type == tail_type:
                    entity = self.decode_entity(text, mapping, head, tail)
                    if head_type == 0:
                        objects.append((entity, head, tail))
                    else:
                        entity_map[head] = entity
                    break
        ...
        if objects:
            ...
            if relations:
                ...
                batch_obj_entities = []
                batch_rel_types = []
                for (obj, obj_head, obj_tail) in objects:
                    obj = self.decode_entity(text, mapping, obj_head, obj_tail)
                    for rel in relations:
                        batch_obj_heads.append([obj_head])
                        batch_obj_tails.append([obj_tail])
                        batch_rels.append([rel])
                        batch_obj_entities.append(obj)
                        batch_rel_types.append(self.id2rel[rel])

                batch_token_ids = np.repeat(token_ids, len(objects) * len(relations), 0)
                batch_segment_ids = np.zeros_like(batch_token_ids)
                sub_head_logits = self.translate_model_sub.predict_on_batch([
                    batch_token_ids, batch_segment_ids, np.array(batch_obj_heads), np.array(batch_obj_tails), np.array(batch_rels)
                ])

                for obj, rel, sub_head_logit in zip(batch_obj_entities, batch_rel_types, sub_head_logits):
                    for h in np.where(sub_head_logit > threshold)[0].tolist():
                        if h in entity_map:
                            sub = entity_map[h]
                            triple_set.add((sub, rel, obj))
